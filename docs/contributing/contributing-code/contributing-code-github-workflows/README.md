...
Each lifecycle phase is a separate workflow step. On failure, inspect the failed step first, then download the `repo-radius-state-e2e-diagnostics` artifact. Successful runs record the saved/restored digest in the job summary and delete only their run-specific state version; the private package and bootstrap version remain.
